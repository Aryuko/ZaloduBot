...
const self = module.exports = {
    ...
    // Saves the global stats letiable to the json file at statsFilePath
    saveStats: function () {
    ...
    initStats: function (guildsArray) {
        ...
        for (let i = 0; i < guildsArray.length; i++) {
            let info = self.indexGuild(guildsArray[i]);
        ...
        let finalTime = (Date.now() - startTime) / 1000.0;
    ...
    indexGuild: function (guild) {
        let info = {"numUsers": guild.members.array().length, "newUsers": 0, "newUsernames": 0, "newNicknames": 0};
        ...
        let statsGuild = stats.guilds[guild.id];

        // For all members in the guild
        let membersArray = guild.members.array();
        for (let j = 0; j < membersArray.length; j++) {
            ...
            let statsMember = statsGuild.members[membersArray[j].id];
            let statsUser = stats.users[membersArray[j].id];
        ...
        let channelsArray = guild.channels.array();
        for (let j = 0; j < channelsArray.length; j++) {
            ...
            if (channelsArray[j].type == "text") {
                ...
                let statsChannel = statsGuild.channels[channelsArray[j].id];
    ...
    getUsersByName: function (name, guild) {
        if (name.length > 0) {
            if (stats.guilds.hasOwnProperty(guild.id)) {
                let statsMembers = stats.guilds[guild.id].members;

                let data = { "foundUsers": {}, "error": { "code": 0, "message": "" } };

                // Check through stats for members on the guild
                for (let memberKey in statsMembers) {
                    // Check through all saved nicknames for the member
                    for (let nickname in statsMembers[memberKey].nicknames) {
                    ...
                    if (stats.users.hasOwnProperty(memberKey)) {
                        for (let username in stats.users[memberKey].usernames) {
    ...
    guildMemberUpdate: function (oldMember, newMember) {
        if (oldMember.nickname != newMember.nickname) {
            let statsGuild = stats.guilds[oldMember.guild.id];
    ...
    userUpdate: function (oldUser, newUser) {
        if (oldUser.username != newUser.username) {
            ...
            let statsUser = stats.users[oldUser.id];
    ...
    guildMemberAdd: function (member) {
        let statsGuild = stats.guilds[member.guild.id];
    ...
    guildCreate: function (guild) {
        let info = self.indexGuild(guild);
    ...
    guildUpdate: function (oldGuild, newGuild) {
        ...
        if (oldGuild.name != newGuild.name) {
            if (stats.guilds.hasOwnProperty(oldGuild.id)) {
                let statsGuild = stats.guilds[oldGuild.id];
